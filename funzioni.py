...
def canny_edge_detection(image, percTl, percTh, mediato):
    ...
    while k < len(pixel_candidati):
        ...
        if (theta[i, j] > 22.5 and theta[i, j] <= 67.5) or (theta[i, j] > -157.5 and theta[i, j] <= -112.5):
            ...
            if G[i, j] < G[i + 1, j + 1] or G[i, j] < G[i - 1, j - 1]:
                # marco i,j come non edge -> bianco
                out[i, j] = 255
        elif (theta[i, j] > -22.5 and theta[i, j] <= 22.5) or (theta[i, j] > 157.5 and theta[i, j] <= 180) or (
                theta[i, j] > -180 and theta[i, j] <= -157.5):
    ...
    lintorno_grande = [[0, 0], [0, 1], [0, 2], [0, 3], [0, 4], [1, 0], [2, 0], [3, 0], [4, 0], [4, 1], [4, 2], [4, 3],
                       [4, 4], [1, 4], [2, 4], [3, 4]]
    k = 0
    while k < len(pixel_candidati):
        i, j = pixel_candidati[k]
...
# Racchiude l'immagine in una linea bianca
def incornicia(image):
    M = len(image)
    N = len(image[0])
    size = image.shape
    if (len(size) == 3):
        print 'a colori:'
    if (len(size) == 2):
        im_incor = np.ones((M + 2, N + 2), dtype=np.uint8) * 255
